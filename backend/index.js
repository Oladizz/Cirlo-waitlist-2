...
app.post('/send-waitlist-email', async (req, res) => {
    const { recipientEmail, name, htmlContent } = req.body;
    console.log('Received request body:', req.body);

    if (!recipientEmail || !htmlContent) {
        return res.status(400).json({ message: 'Recipient email and HTML content are required.' });
    }

    try {
        const personalizedContent = htmlContent.replace('{{name}}', name || 'there');
